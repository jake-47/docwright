...
This script is about 2176 lines long and the result of vibecoding with Opus 4.7, 4.8, 5, a few times with Fable 5, and 150+ iterations (for real).
I hate Anthropic and hope that local LLMs become a viable alternative.
Anyway I've tested all the user-facing functions, and they work.
I can't code, so please review this script if you can.
If it's too long, then please consider reviewing the [super-simple script](./borg-super-simple.md) which contains only the core functions for easy review.
Hope there's nothing malicious.
...
# `backup` backs up every repo, `backup <repo>` one, `backup --help` lists the
# commands. If borg itself is missing, the first command that needs it installs
# borgbackup with apt, so a fresh machine needs only this script.
...
readonly SUBCOMMANDS=(init extract list check pass-change rename)
...
# The one remedy for an unwritable repo directory, so archive, extract, check and
# list all say the same thing. Ownership is the usual cause: a drive filled by
# another machine or another user hands you someone else's uid, and borg needs
# the directory itself writable, not just its contents.
_unwritable_hint() {
    warn "  inspect:  findmnt -no FSTYPE,OPTIONS $MOUNT_BASE/<drive>   and   ls -ld <repo dir>"
    warn "  take it:  sudo chown -R \"\$(id -un):\$(id -gn)\" $MOUNT_BASE/<drive>"
}

# Decide the lock mode for a read of repo dir $1: bypass borg's lock when the
# directory is not writable (another user's files, or read-only media), warning
# once per directory so a scan across several drives does not repeat itself. The
# dedup is per process, and the extract scan and the extract itself run in
# separate subshells, so one directory can still account for two warnings in a
# run; that is the ceiling, not a per-probe repeat. Read-only callers only
# (list, extract, check); a write path must hold the lock, never this.
_set_read_lock() {
    local repo="$1"
    if [[ -w "$repo" ]]; then
        RO_LOCKOPT=()
...
# A run on a machine that already has borg pays one command -v and returns.
ensure_borg() {
...
# the name as a symlink and have this exec 9> truncate a file you own. (This is a
# deliberate departure from the usual runtime-dir lock location.)
_acquire_lock() {
...
# the subcommand instead of archiving the repo. Reserve the visible dispatch words,
# the help flags, and the hidden dispatch words (_emit-pass, _pass-names,
# _selfcheck) for the CLI. Drive labels are never dispatched, so this guards
...
backup_drives() {
    ...
    if [[ "$1" == all ]]; then
        ...
        for d in $ALL_DRIVES; do
            ...
            case " $seen " in *" $d "*) die "config: ALL_DRIVES lists drive '$d' twice" ;; esac
            seen+="$d "
        done
    ...
    for d in "$@"; do
        ...
        case " $seen " in *" $d "*) die "config: backup_drives lists drive '$d' twice" ;; esac
        seen+="$d "
    done
...
parse_config() {
    ...
    # The passphrase file's name carries no meaning: _pass_source_kind reads the
    # format out of the file itself, so call it what you like. Empty (or a path
    # with no file) -> borg prompts, the v88 fallback. The one real constraint is
    # that a set path be space-free, because borg splits BORG_PASSCOMMAND itself.
...
# anything. The test is for the thing that actually distinguishes the two files:
# a plaintext passphrase file carries set_pass lines in the clear and ciphertext
# does not. v113 tested the first byte for the high bit instead, which classified
# any file opening with a UTF-8 BOM or a non-ASCII character as encrypted; this
# does not care about encoding, BOMs or editor artifacts. The suffix decided it
# through v112, which meant a mis-suffixed encrypted file was read as plaintext
# and then rewritten as plaintext by _pass_write. The script never creates this
# file, so the format is always there to observe rather than something that has
# to be declared. Absent or unset path -> none, and the callers that need a file
# refuse. grep on ciphertext is harmless; a false 'plain' would need the literal
# bytes of a set_pass line at the start of a line inside an encrypted blob, and
# the eval downstream would fail loudly rather than expose anything.
_pass_source_kind() {
    [[ -n "$PASSPHRASE_PATH" && -e "$PASSPHRASE_PATH" ]] || { printf 'none\n'; return; }
    # A UTF-8 BOM sits before the first line's first character, so it has to be
    # allowed for explicitly or a single-repo file whose only set_pass line is
    # line 1 reads as ciphertext.
    local bom=$'\xef\xbb\xbf'
    if LC_ALL=C grep -qaE "^(${bom})?[[:space:]]*set_pass[[:space:]]" -- "$PASSPHRASE_PATH"; then
        printf 'plain\n'; return
    fi
    # No set_pass line. Call it encrypted only if it looks like an OpenPGP
    # message, armored or binary. A file that is merely missing its set_pass
    # lines, or is empty, stays plaintext so the caller reports that rather than
    # a decrypt failure for a file that was never encrypted.
...
# documented "borg asks" fallback is live for this run. Every command except
# archive already honoured it, by emitting no BORG_PASSCOMMAND and letting borg
# ask; archive skipped instead, which is the v112 bug this replaces. Tty-gated so
# an unattended run still fails closed rather than stalling on a prompt nobody can
# answer (spec rule 50). The prompt is per borg invocation, not per repo: one repo
# on one drive asks for create, the two verification lists, prune, compact and
# check, so a multi-repo pool is a lot of typing. That is the price of keeping the
# passphrase off disk and out of this process; the alternative is a passphrase file.
...
_archive_pass_preflight() {
    if _pass_prompt_mode; then
        say "no passphrase file configured; borg will ask, once per operation per repo per drive"
        return 0
    fi
    # Check the file's permissions here, in the parent, before the names child
    # reads it. require_private fires either way, but from inside the child it
    # arrives as three stacked messages (the mode complaint, "could not read
    # passphrases", then "refusing to archive") with nothing on stdout, which
    # reads as the run having done nothing at all. A file created by a redirect or
    # an editor is mode 644 under the default umask, so this is the first thing a
    # new passphrase file trips.
...
_validate_drives_list() {
    ...
    for d in $drives; do
        ...
        _validate_segment "drive" "$d"
        case " $seen " in *" $d "*) die "--drives lists drive '$d' twice" ;; esac
        seen+="$d "
    done
}
...
_archive_one_drive() {
    ...
    # borg's first act on a write is to create a lock file inside the repo dir, so
    # an unwritable repo fails with a Python traceback several steps in. Say it
    # plainly here instead. The preflight already refused the run if no drive had
    # a writable repo; this catches the mixed case, where some drives are fine.
    if [[ ! -w "$BORG_REPO" ]]; then
        warn "$repo on $drive is not writable ($BORG_REPO)"
        _unwritable_hint
        return 1
    fi
    local pc; pc=$(_repo_passcommand "$repo")
    ...
    # borg create is the longest step by far and printed nothing until now, while
    # every extract call shows progress. Progress goes to stderr, so gate it on
    # stderr being a terminal: interactive runs get feedback, cron logs stay clean.
    local -a progress=()
    ...
    # borg and the verification below both accept it. borg create --json already
    # reported the file count, so this costs nothing and covers every repo; until
    # v113 it ran only for include_only repos and spent two extra borg calls, the
    # second of which pulled the archive's entire file list into a variable to ask
    # whether it was empty. A folder-scoped include_only that matches nothing only
    # empties its one folder, which a whole-archive count still cannot see; verify
    # those with --dry-run.
    ...
    # Verify before retention, not after. Prune deletes archives and compact frees
    # their space, so running them first would destroy known-good history on the
    # assumption that the archive just written is good, and test that assumption
    # afterwards. On a failing drive that trades real backups for a bad one, and
    # compact makes it unrecoverable. Nothing is pruned unless the new archive
    # both verifies and holds files.
    local check_out="" check_rc=0
    if [[ -n "$pc" ]]; then
        check_out=$(borg check --archives-only --last 1 2>&1) || check_rc=$?
    else
        # With no passcommand borg asks for the passphrase on stderr, so capturing
        # stderr here would hide the prompt and the run would look hung. Let it
        # through and give up the captured reason for this one case.
    ...
    if (( empty_archive )); then
        # Structurally valid but empty: the tripwire above already warned why.
        # Report unverified so an empty archive is not counted as a good backup,
        # and keep retention off so it cannot rotate a real archive out.
        warn "retention skipped for $repo on $drive; no archive was pruned"
        return 2
    fi
...
# Set to 1 once a run has printed its summary, so the EXIT trap below stays quiet
# on a normal exit. The trap must not disturb the exit status, so it never calls
# exit and never returns non-zero.
_ARCHIVE_REPORTED=0
_archive_exit_notice() {
    local rc=$?
    if (( rc != 0 && _ARCHIVE_REPORTED == 0 )); then
        printf 'backup failed before it could report; the reason is on stderr above (exit %d)\n' "$rc"
    fi
    return 0
}
...
_archive_repo() {
    local repo="$1"
    # borg treats a source path that is not there as a warning, and the create
    # step below deliberately maps borg's warning code to success so a dotfile
    # changing mid-run is not a failure. Those two together would let a deleted,
    # renamed or unmounted folder produce a short archive that reports as good and
    # then rotates a real one out under retention. Check first and refuse the repo
    # instead: the run loses one backup, where proceeding loses an old one. Symlink
    # targets are not resolved here, deliberately; a symlinked source is a separate
    # open question and -e follows the link either way.
    local missing_src=() s
    ...
    if (( ${#missing_src[@]} > 0 )); then
        warn "repo $repo not archived: configured folder(s) missing: ${missing_src[*]}"
...
cmd_archive() {
    ...
    # that died on a config or passphrase problem was indistinguishable from a run
    # that never happened if you were not watching stderr. This is the backup log
    # for a cron-driven tool, so the outcome has to be answerable from one stream.
    _ARCHIVE_REPORTED=0
    ...
    # loop below apply. Mounted is not enough: borg's first act on a write is to
    # create a lock file inside the repo directory, so a mounted-but-unwritable
    # repo (read-only mount, a directory owned by another uid, a vfat or exfat
    # mount handing you someone else's uid) fails several seconds and one
    # passphrase later with a Python traceback. Check writability here so nothing
    # is solicited for work that cannot succeed. Archive cannot fall back the way
    # check and extract do: _set_read_lock bypasses locking for reads, and there
    # is no such bypass for a write. If repos would archive but no drive offers a
    # writable repo, fail here, before decrypting the passphrase (for a GPG file
    # that is a prompt or a hardware-token touch). If nothing would archive at all
    # (every repo archive-off or awaiting data), skip the passphrase entirely and
    # let the run below report that and exit.
    local _r _d _rp _has_target=0 _any_drive=0 _any_writable=0
    local -a _want_drives=() _unwritable=()
    for _r in "${run_repos[@]}"; do
        ...
        for _d in ${REPO_DRIVES[$_r]}; do
            _want_drives+=("$_d")
            mountpoint -q "$MOUNT_BASE/$_d" || continue
            _any_drive=1
            _rp=$(_repo_path "$_d" "$_r")
            if [[ ! -d "$_rp" ]]; then
                continue                      # reported per drive by _archive_one_drive
            elif [[ -w "$_rp" ]]; then
                _any_writable=1
            else
                _unwritable+=("$_r on $_d")
            fi
        done
    done
    if (( ${#_unwritable[@]} > 0 )); then
        warn "repo directory not writable: ${_unwritable[*]}"
        _unwritable_hint
    fi
    ...
    if (( _has_target && _any_drive && ! _any_writable )); then
    ...
    # before it makes you touch anything. init, pass-change and rename already
    # lock first. Everything above this point is read-only, so nothing is lost by
    # holding the lock from here.
    _acquire_lock
    ...
    # passphrase while the run reports it came from the passphrase file or a
    # prompt. Not neutralised, since setting one deliberately is legitimate, but
    # never silent: the extract path warns the same way.
    ...
    (( _has_target )) && _archive_pass_preflight "${run_repos[@]}"
    ...
    for repo in "${run_repos[@]}"; do
        ...
        # the preflight. In prompt mode there is nothing to store, so run it and
        # let borg ask.
...
# The per-drive warnings here are the only report of an unmounted or repo-less
# drive, so callers stay quiet about it. Empty output means no copy is reachable.
_extract_candidates() {
    local repo="$1" d probe
    # shellcheck disable=SC2086  # space-joined validated drive tokens; intentional split
    for d in ${REPO_DRIVES[$repo]}; do
        if ! mountpoint -q "$MOUNT_BASE/$d"; then warn "$d not mounted"; continue; fi
        probe=$(_repo_path "$d" "$repo")
        if [[ ! -d "$probe" ]]; then warn "no $repo repo on $d"; continue; fi
        printf '%s\n' "$d"
    done
    return 0
}

# Resolve the spec against one drive's copy and print the archive name. The borg
# environment is built inside a subshell so BORG_REPO, BORG_PASSCOMMAND and
# RO_LOCKOPT never carry over to the next drive.
_archive_on_drive() {
    ...
    (
        ...
        local pc; pc=$(_repo_passcommand "$repo")
        if [[ -n "$pc" ]]; then local -x BORG_PASSCOMMAND="$pc"; fi
...
# caller can fall through to the next copy; the borg environment is subshelled
# for the same isolation reason as _archive_on_drive.
_extract_from_drive() {
    ...
    (
        ...
        if (( ${#paths[@]} > 0 )); then
            ...
            local p; for p in "${paths[@]}"; do say "  $p"; done
            exit 0
        fi
...
# lines. Copies whose spec will not resolve are dropped here rather than at
# extract time, so a stub or archive-less repo directory can no longer win the
# pick. Sorting is by archive name descending because names come from borg's
# {now} and so sort chronologically; the config-order index breaks ties, which is
# every tie when the spec is a literal archive name. Prints nothing if no copy
# resolves. Nothing is said about copies being out of step: they always are
# mid-run, and `backup list` shows the real state on demand.
_extract_plan() {
    local repo="$1" spec="$2"; shift 2
    local -a cand=("$@")
    local -a found=()
    ...
    printf '%s\n' "${found[@]}" | sort -t$'\t' -k1,1r -k2,2n | cut -f1,3
}
...
cmd_extract() {
    ...
    if (( all_repos )); then
        ...
        local rc=0 r j rdest rarchive rdrive
        local -a restored=() skipped=() failed=() rcand=() rplan=()
        for r in "${REPOS[@]}"; do
            rcand=()
            mapfile -t rcand < <(_extract_candidates "$r")
            if (( ${#rcand[@]} == 0 )); then skipped+=("$r (no mounted drive)"); continue; fi
            ...
            rplan=()
            mapfile -t rplan < <(_extract_plan "$r" "$spec" "${rcand[@]}")
            if (( ${#rplan[@]} == 0 )); then skipped+=("$r (no usable archive on any copy)"); continue; fi

            rdest="$RESTORE_PATH/$r"
            for (( j = 0; j < ${#rplan[@]}; j++ )); do
                rarchive="${rplan[j]%%$'\t'*}"; rdrive="${rplan[j]##*$'\t'}"
                if _extract_from_drive "$r" "$rdrive" "$rarchive" "$in_place" "$rdest"; then
                    restored+=("$r"); break
                fi
                warn "extraction of $r from $rdrive failed; see borg output above"
                if (( in_place )); then
                    warn "whatever borg wrote before the failure is already in place; the rest of $r was not restored"
                else
                    warn "$rdest may now hold a partial $r tree"
                fi
                if (( j + 1 < ${#rplan[@]} )); then
                    warn "falling through to the next copy of $r; it overwrites what the failed attempt left"
                else
                    rc=1; failed+=("$r")
                fi
            done
    ...
    # Every mounted drive that has this repo. One copy failing no longer ends the
    # restore: the copies are ordered and tried in turn.
    local -a cand=()
    mapfile -t cand < <(_extract_candidates "$repo")
    (( ${#cand[@]} > 0 )) || die "no mounted drive has the $repo repo"
    ...
    if [[ -n "${BORG_PASSPHRASE:-}${BORG_PASSPHRASE_FD:-}${BORG_PASSCOMMAND:-}" ]] || _repo_has_pass "$repo"; then
        ...
        (( ${#plan[@]} > 0 )) || die "no copy of $repo holds a usable archive; run: backup check $repo"
    else
        (( ${#cand[@]} == 1 )) \
            || warn "no stored passphrase for $repo; keeping to the first copy (${cand[0]}) rather than prompting once per drive"
        if _spec_lists_first "$spec"; then
    ...
    local rc=1 i archive drive
    for (( i = 0; i < ${#plan[@]}; i++ )); do
        archive="${plan[i]%%$'\t'*}"; drive="${plan[i]##*$'\t'}"
        if _extract_from_drive "$repo" "$drive" "$archive" "$in_place" "$dest" ${paths[@]+"${paths[@]}"}; then
            rc=0; break
        fi
        ...
        if (( i + 1 < ${#plan[@]} )); then
            warn "falling through to the next copy of $repo; it overwrites what the failed attempt left"
        fi
    done
...
## ─── list: what each drive actually holds ────────────────────────────
# Archives on one drive's copy, oldest first as borg emits them, at most $3.
# Subshelled for the same borg-env isolation as the extract helpers.
_list_one() {
    local repo="$1" dir="$2" last="$3"
    (
        local -x BORG_REPO="$dir"
        _set_read_lock "$dir"
        local pc; pc=$(_repo_passcommand "$repo")
        if [[ -n "$pc" ]]; then local -x BORG_PASSCOMMAND="$pc"; fi
        borg list ${RO_LOCKOPT[@]+"${RO_LOCKOPT[@]}"} --last "$last" --short
    )
}
...
cmd_list() {
    ...
    while (( $# > 0 )); do
        case "$1" in
            -l|--last)
                (( $# >= 2 )) || die_usage "--last needs a value"
                [[ "$2" =~ ^[1-9][0-9]*$ ]] || die_usage "--last needs a positive whole number, got '$2'"
                last="$2"; shift 2 ;;
            --last=*) die_usage "use '--last <value>'; '--last=value' is not supported" ;;
            -*) die_usage "unknown flag: $1" ;;
            *)  [[ -z "$only" ]] || die_usage "list takes at most one repo; got '$1' as well as '$only'"
                only="$1"; shift ;;
        esac
    done
    ensure_borg; need mountpoint
    ...
    local -a todo=()
    if [[ -n "$only" ]]; then
        _repo_declared "$only" || die "$only is not a configured repo (check $CONFIG_FILE)"
        todo=("$only")
    else
        todo=("${REPOS[@]}")
    fi

    local repo drive dir rc=0 shown listing i
    local -a arch=()
    for repo in "${todo[@]}"; do
        say "$repo"
        shown=0
        if ! _repo_has_pass "$repo" \
            ...
            warn "no stored passphrase for $repo; borg will prompt once per drive below."
        fi
        # shellcheck disable=SC2086  # space-joined validated drive tokens; intentional split
        for drive in ${REPO_DRIVES[$repo]}; do
            mountpoint -q "$MOUNT_BASE/$drive" || continue
            dir=$(_repo_path "$drive" "$repo")
            [[ -d "$dir" ]] || continue
            shown=1
            say "  $drive"
            if ! listing=$(_list_one "$repo" "$dir" "$last"); then
                say "    cannot read the repo here; try: backup check $repo"
                rc=1
                continue
            fi
            arch=()
            mapfile -t arch <<< "$listing"
            [[ -n "${arch[-1]:-}" ]] || unset 'arch[-1]'
            if (( ${#arch[@]} == 0 )); then
                say "    no archives yet"
                continue
            fi
            # borg hands back the last N oldest-first; newest-first reads better
            # next to 'extract <repo> -N', which counts back from the newest.
            for (( i = ${#arch[@]} - 1; i >= 0; i-- )); do
                say "    ${arch[i]}"
            done
        done
        (( shown )) || { say "  no mounted drive has this repo"; rc=1; }
    done
    exit "$rc"
}

## ─── check: deep-verify a repo on every drive ─────────────────────────
# Fresh function scope per call so BORG_REPO and BORG_PASSCOMMAND never leak between repos.
_check_one() {
    local repo="$1" dir="$2"
    local -x BORG_REPO="$dir"
...
backup: encrypted, versioned borg backups to USB drives
  ...
  backup extract <repo-path>   restore by path, no config needed (borg prompts)
  backup list [repo]           show each mounted drive's archives, newest first
                               (-l N, --last N; default 10)
...
If borg is not installed, the first command that needs it installs borgbackup with apt.
...
main() {
    ...
    # Bare invocation (no args) backs up every repo; an argument that is the empty
    # string (e.g. an unset "$REPO" in a cron line) is a mistake, not a full run,
    # so it is rejected below rather than escalated to every repo.
    (( $# == 0 )) && { cmd_archive; exit "$?"; }
    case "${1:-}" in
        ...
        "")          die_usage "empty repo name; run 'backup' with no arguments to back up every repo, or name a repo" ;;
